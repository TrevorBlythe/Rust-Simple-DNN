...
pub struct Sig {
    ...
    pub costs: Vec<f32>, //costs for each "neuron/node/input data" cost[0] is for last layers out_data[0]
    pub in_size: usize,
    pub out_size: usize,
}

pub fn new(size: usize) -> Box<Sig> {
    return Box::new(Sig {
        costs: vec![0.0; size],
...
impl GenericLayer for Sig {
    ...
    fn get_params_and_grads(&mut self) -> (&mut Vec<f32>, &mut Vec<f32>) {
        panic!("shoudnt ever call this on a siggy");
    }
    ...
    fn backward_data(&mut self, _data_in: &Vec<f32>, expected: &Vec<f32>) {
        //we dont need the input data because of some optimization i found online yay!
        ...
        for j in 0..self.in_size {
            ...
            self.costs[j] += z * (self.out_data[j] * (1.0 - self.out_data[j])); //err * sigprime(input of layer)
        }
    }
    ...
    fn backward_costs(&mut self, _data_in: &Vec<f32>, costs: &Vec<f32>) {
        ...
        for j in 0..self.in_size {
            self.costs[j] += costs[j]
                * (self.out_data[j] * (1.0 - self.out_data[j]));
        }
    }

    fn forward_data(&mut self, data: &Vec<f32>) {
        for i in 0..self.out_data.len() {
            self.out_data[i] = 1. / (1. + (-data[i]).exp());
        }
    }
}
